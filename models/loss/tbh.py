...
class TBHLoss(nn.Module):
    ...
    def forward(self, feats, codes, rec_feats, discs):
        rec_loss = torch.sum((rec_feats - feats).pow(2)) * 0.5  # tf.nn.l2_loss is sum(x ** 2) / 2
        divisor = 1
        if self.rec_method == 'batch':
            divisor = feats.size(0)
        elif self.rec_method == 'all':  # including bits
            divisor = feats.size(0) * feats.size(1)
