...
class OrthoHashLoss(BaseLoss):
    def __init__(self,
        ...
        assert multiclass_loss in ['bce', 'imbalance', 'label_smoothing', 'label_smoothing_unscaled']
    ...
    def forward(self, logits, code_logits, labels, onehot=True):
        if self.multiclass:
            ...
            elif self.multiclass_loss in ['label_smoothing', 'label_smoothing_unscaled']:
                log_logits = F.log_softmax(margin_logits, dim=1)
                if 'unscaled' not in self.multiclass_loss:
