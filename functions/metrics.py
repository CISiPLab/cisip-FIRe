...
def calculate_mAP(db_codes, db_labels,
    ...
    for R in Rs:
        ...
        logging.info(f'Start Sorting (R = {R})')
        timer.tick()
        ...
        # calculate mAP
        if using_id:
            output = compute_mAP_score_for_id(dist, topk_ids,
                                              db_id, test_id, ground_truth,
                                              R, return_dist)
        else:
            output = compute_mAP_score(dist, topk_ids,
                                       db_labels, test_labels, R, onehot, old_eval, return_dist)
...
def compute_mAP_score(dist,
    ...
    for i in pbar:
        if onehot:
            ...
            label = test_labels[i]
            idx = topk_ids[i, :]
            imatch = (db_labels[idx[0: R]] == label) > 0

        rel = np.sum(imatch)
        Lx = np.cumsum(imatch)
        Px = Lx.astype(float) / np.arange(1, R + 1, 1)  # ap += num_correct / (i + 1)
    ...
    if return_dist:
        return mAP, torch.stack(Dists).cpu().numpy()
    return mAP


def compute_mAP_score_for_id(dist,
                             topk_ids,
                             db_id,
                             test_id,
                             ground_truth,
    ...
    for i in pbar:
        ...
        APx.append(np.sum(Px * imatch) / len(all_id_should_retrieve))
